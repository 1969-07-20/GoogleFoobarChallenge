...
def solution(M, F):
    step, mach, facula= 0, long(M), long(F)
    M = long(M)
    F = long(F)
    try:
        if mach == facula or mach <= 0 or facula <= 0: raise ValueError('Incorrect number of bomb types encountered')
        while True:
            if mach <= 0 or facula <= 0: raise ValueError('Zero or less bomb types encountered')
    ...
    if mach == 1 and facula == 1 and step >= 0: return str(step)
